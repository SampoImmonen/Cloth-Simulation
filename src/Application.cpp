...
void Application::GLFWCallbackWrapper::MouseCallback(
    GLFWwindow* window,
    double positionX,
    double positionY) {
...
void Application::GLFWCallbackWrapper::FramebufferResizeCallback(
    GLFWwindow* window,
    int width,
    int height) {
...
void Application::GLFWCallbackWrapper::scrollCallback(
    GLFWwindow* window,
    double xoffset,
    double yoffset) {
...
const std::vector<uint32_t> tri_indices = {0, 1, 2, 1, 2, 3};
...
void Application::init() {
  ...
  auto shaderdir = basepath.parent_path().string() + "/src/shaders/";

  Shader shader(shaderdir + "basicshader.vert", shaderdir + "basicshader.frag");
  m_shaders.push_back(shader);
  Shader clothRender(shaderdir + "ClothRender.vert", shaderdir + "ClothRender.frag");
  m_shaders.push_back(clothRender);
  Shader ShadowMapShader(
      shaderdir + "ShadowDepthShader.vert", shaderdir + "ShadowDepthShader.frag");
  m_shaders.push_back(ShadowMapShader);
  Shader NormalVisualizationShader(
      shaderdir + "NormalShader.vert",
      shaderdir + "NormalShader.frag",
      shaderdir + "NormalShader.geo");
  m_shaders.push_back(NormalVisualizationShader);

  // compute shaders
  Shader ClothPositionShader(shaderdir + "ClothCompute.comp");
  m_computeShaders.push_back(ClothPositionShader);
  Shader ClothNormalShader(shaderdir + "ClothNormals.comp");
  m_computeShaders.push_back(ClothNormalShader);

  // compute shaders for trapezoid integrator
  Shader TrapezoidShaderPart1(shaderdir + "trapezoid_step1.comp");
  m_trapezoidShaders.push_back(TrapezoidShaderPart1);
  Shader TrapezoidShaderPart2(shaderdir + "trapezoid_step2.comp");
  m_trapezoidShaders.push_back(TrapezoidShaderPart2);
  Shader TrapezoidShaderPart3(shaderdir + "trapezoid_step3.comp");
  ...
  m_planeMaterial.m_normalMap =
      std::make_shared<Texture2D>(Texture2D(texturedir + "brickwall_normal.jpg"));
  // imgui
  ImGui::CreateContext();
...
void Application::initBuffers() {
  ...
  glBufferData(
      GL_ARRAY_BUFFER,
      tx_coordinates.size() * sizeof(glm::vec2),
      &tx_coordinates[0],
      GL_STATIC_DRAW);
...
void Application::render() {
  ...
  glm::mat4 projection =
      glm::perspective(glm::radians(m_camera.Zoom), (float)m_width / m_height, 0.1f, 100.0f);
...
void Application::clothMaterialUI() {
  if (ImGui::CollapsingHeader("Cloth Material")) {
    ...
    if (ImGui::ImageButton(
            (void*)(intptr_t)clothAlbedoID, ImVec2(50, 50), ImVec2(0, 1), ImVec2(1, 0))) {
